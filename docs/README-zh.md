...
`ftime` 是为 Context Recovery（作业上下文恢复）而设计的。它的主要目标不是只提取最新的那 1 个路径，而是帮助你快速重新建立对单个文件夹当前工作上下文的判断。

`Active`、`Today`、`This Week`、`History` 这些时间桶会把最近活动切成更容易扫读的块，比单纯的时间排序列表更适合快速恢复上下文。

## Non-Goals

`ftime` 不是 `fd`、`find`、`eza` 或 `git status` 的替代品。

- 递归搜索不是它的主要目标。需要搜索深层目录时，请用 `fd` 或 `find`。
- 丰富的 Git 状态查看不是它的主要目标。需要看 tracked / untracked 时，请用 `git status`。
- 破坏性操作不在范围内。`ftime` 会保持只读。
- 通用文件发现或一次性提取不是它的主要目标。它先帮助你恢复上下文，再让你更容易找到下一步要查看的对象。
...
这个提示只是辅助信息；父目录自身的 bucket 和排序仍然以该目录自己的 `mtime` 为准。
...
| `fd` | 递归搜索和过滤 | 天生就是递归工具 |
| `bat` | 阅读文件内容 | 不是目录活动视图 |
| `ftime` | 恢复单个文件夹的工作上下文 | 时间桶 + 大小一眼可见 |
