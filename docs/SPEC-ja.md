# ftime v1.0.4 仕様書（日本語版・詳細）

本ドキュメントは `SPEC-v1.0.md` の日本語拡張版です。v1.0 の現行CLI契約を対象とし、将来拡張（Git連携等）は参考情報として別節にまとめます。
...
- **非対象**:
  ...
  - 再帰的ディレクトリ走査（パフォーマンスと意図的なシンプルさのため v1.0 でも未対応）。
  - 由来判定ラベルは `Fresh`（約5分以内）以外は未対応（Imported 等は将来案）。
...
- **エントリ収集**:
  ...
  - シンボリックリンクのターゲット表示: `read_link` が成功すれば、リンク先が存在しなくても（danglingでも）ターゲット文字列は表示する。`read_link` が失敗した場合のみ `<unresolved>` と出力する。
  - パーミッションエラー: そのエントリをスキップして処理継続（必要なら将来 `--verbose` で警告表示を検討）。
  - dotfiles: 先頭が `.` の名前もデフォルトで含める。不要なら `--exclude-dots` で除外。
...
| --- | --- | --- |
| 🔥 Active | `now - mtime < 1時間` | `main.rs 12 mins ago` |
| ☀️ Today | `mtime >= 今日 00:00:00` | `notes.md 3 hours ago` |
| 📅 This Week | `now - mtime < 7日` | `report.pdf 3 days ago` |
| 💤 History | 上記以外 | `archive.zip 2024-11-02` |

- **未来時刻**: システム時計のずれ等で未来になっている場合も Active に入れる（作業中ファイルを見逃さないため）。表示は `+Ns [Skew]` / `+Nm [Skew]`。
...
## 5. 出力形式（TTYモード）
- **ヘッダー**: バケットごとにアイコン＋名称を太字で表示。例 `🔥 Active`。
- **エントリ行**: `"  • {名前} | {サイズ} | {相対時間}"`。  
...
## 8. 環境変数
- `NO_COLOR`: 設定されていればカラー無効。空文字でも無効扱い（no-color標準とは異なるが仕様として固定）。
- `FTIME_FORCE_TTY`: 設定されていればTTYスタイルを強制。CIやスナップショットテストで便利。

補足: `--icons` は icons feature 無効バイナリでもエラーにせず無視（no-op）。PATH がファイルのときは先にエラー終了するため `--ext` は適用されない。`symlink_target` は symlink かつ解決成功時のみ JSON に出力し、失敗または非symlinkの場合は省略。`FTIME_IGNORE` でグローバル ignore ファイルを指定でき、`--no-ignore` で無効化できる。`label` フィールドは現在 `Fresh` のみで、約5分以内の更新時刻に付与。JSON は Fresh 時のみ `label` を出力し、該当しない場合は省略。`--no-labels` で無効化可能。  

## 9. 対応プラットフォーム
- 主対象: Linux（主要ディストリ）, macOS。
- Windows: ビルド可能だが、シンボリックリンクとANSIカラーは端末依存。
...
- テーマカラーのカスタマイズ、出力フォーマット指定（CSVや任意フォーマット選択）は現状非対応。JSON は `--json` で提供。
...
- 過剰な機能追加はUXを複雑にするため、v1.0は最小限の操作・依存・出力に絞る。
...
- 隠しファイルも含めて履歴を広げたい場合は `ftime -a`。隠しファイルを除きたい場合は `ftime -a --exclude-dots`。
...
- **Q: mtime以外（atime/btime）を使えない？**  
  A: v1.0もmtime固定。btimeはプラットフォーム差が大きく、仕様を安定させるため採用していない。
- **Q: 20件制限を変更したい**  
...
## 20. トラブルシューティング
- **何も表示されない**: 実ディレクトリが空、または `--exclude-dots` / `--ext` / ignore で全件除外されている可能性。フィルタを外して再確認する。
- **Historyだけ多すぎて把握できない**: デフォルトでは折り畳まれている。`-a` で展開し、`head -n` で絞る。
- **タイムゾーンが合わない**: `date` コマンドでシステム時刻を確認。Dockerコンテナ等でTZが異なる場合は環境変数`TZ`を設定する。
- **シンボリックリンクが `<unresolved>` になる**: `read_link` に失敗している可能性（権限や特殊なファイルシステム等）。`ls -l` でターゲット表示や権限を確認。

## 21. 将来版へのヒント（メタ）
- Git連携を入れる場合は `FileEntry` に `git_status` を追加し、ビュー層で色分けを行うと拡張しやすい。現在のバケットロジックはそのまま流用できる。
- CSVなどの別出力を追加する場合、パイプモードのフォーマットを壊さないよう `--format csv` のような明示オプションにするのが安全。
...
if exclude_dots && name starts_with '.': continue
