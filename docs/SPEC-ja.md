# ftime v2.0.0 仕様書（日本語版・詳細）

本ドキュメントは `SPEC-v2.0.md` の日本語拡張版です。v2.0 の現行CLI契約を対象とし、将来拡張（Git連携等）は参考情報として別節にまとめます。
...
- **非対象**:
  ...
  - 再帰的ディレクトリ走査（パフォーマンスと意図的なシンプルさのため v2.0 でも未対応）。
...
- **Q: mtime以外（atime/btime）を使えない？**  
  A: v2.0もmtime固定。btimeはプラットフォーム差が大きく、仕様を安定させるため採用していない。
- **Q: 20件制限を変更したい**  
