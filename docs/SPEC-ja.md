...
本ドキュメントは `docs/SPEC-v1.0.md` の日本語拡張版です。v1.0 のスコープに限定し、将来拡張（Git連携等）は参考情報として別節にまとめます。
...
- **非対象**:
  ...
  - 再帰的ディレクトリ走査（パフォーマンスと意図的なシンプルさのため v1.0 では未対応）。
...
- **Q: mtime以外（atime/btime）を使えない？**  
  A: v1.0はmtime固定。btimeはプラットフォーム差が大きく、仕様を安定させるため採用していない。
- **Q: 20件制限を変更したい**  
