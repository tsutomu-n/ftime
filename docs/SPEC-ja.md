...
補足: `--icons` は icons feature 無効バイナリでもエラーにせず無視（no-op）。PATH がファイルのときは先にエラー終了するため `--ext` は適用されない。`symlink_target` は symlink かつ解決成功時のみ値を持ち、失敗または非symlinkの場合は `null`。`FTIME_IGNORE` でグローバル ignore ファイルを指定でき、`--no-ignore` で無効化できる。`label` フィールドは現在 `Fresh` のみで、約5分以内の更新時刻に付与。`--no-labels` で無効化可能。  
