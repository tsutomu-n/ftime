...
この文書は `ftime` の CLI 契約を日本語で引けるようにしたリファレンスです。`ftime` は 1 フォルダの Context Recovery（作業文脈の再構築）を助けるための read-only CLI で、時間バケットで最近の文脈を整えてから次に見る対象を見つけやすくします。導入は `README-ja.md`、使い方の流れは `USER-GUIDE-ja.md`、英語の正本は `CLI.md` を参照してください。
...
- 再帰検索。深い階層の探索は `fd` や `find` 向きです。
- tracked / untracked を含む Git 状態確認。`git status` 向きです。
- 破壊的操作。`ftime` は読み取り専用を維持します。
- 1 件だけを汎用的に抜き出すこと。`ftime` はまず時間バケットで文脈を整える側に寄っています。
