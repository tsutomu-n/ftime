...
本書は `docs/CLI.md` を日本語で拡充したものです。コマンドの書式、オプションや環境変数の意味、出力例、利用シナリオ別の使い方、トラブルシューティングを7000文字超でまとめています。仕様の根拠は `SPEC-v0.1.md` / `SPEC-ja.md` にあり、本書はユーザーが迷わず使うための実践ガイドです。
...
- 引数は1つのみ。複数指定はv0.1では未サポート（最初の1つだけ読む仕様は採用していない）。
...
## 3. 環境変数
- `NO_COLOR`: 値を問わず設定されていればカラーを完全無効化。TTYでも無色になる。
...
## 11. よくある質問（FAQ）
- **Q: 再帰はできますか？**  
  A: v0.1では深さ1のみ。大量ディレクトリでも高速を保つための意図的制限です。
- **Q: 20件以上見たい**  
...
## 16. バージョンポリシー
- v0.1 はCLIインターフェースを安定化させるフェーズ。互換性を重視し、オプション追加時は後方互換に配慮する。
