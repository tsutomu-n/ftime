...
注: 本書は「v1.0.0 までに何を固めたか／何を先送りしたか」を示す historical planning memo として残しています。
...
- [x] その他、明らかにノイズとなる一般的なシステムファイルの追加候補を整理する。
  候補（未採用）: `desktop.ini`（Windowsのフォルダ設定）、`.Trash` / `.Trash-<uid>`（freedesktop.org Trash Spec の標準ディレクトリ）
