...
`ftime` は Context Recovery（作業文脈の再構築）を助けるためのツールです。まず 1 フォルダの最近の動きを時間バケットで見渡して文脈を整え、そのあとで次に見る対象を絞りやすくします。

一番新しい 1 件だけを抜き出すことより、`Active / Today / This Week / History` の塊で状況を掴み直すことを優先しています。
...
- 深い階層の再帰検索。`fd` や `find` のほうが向いています。
- tracked / untracked を含む Git 状態確認。`git status` のほうが向いています。
- 削除や移動などの破壊的操作。`ftime` は読み取り専用です。
- 1 件だけを機械的に抽出する用途。そうした抽出そのものより、まず文脈を整える側に寄っています。
