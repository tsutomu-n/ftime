...
`ftime` は Context Recovery（作業文脈の再構築）のために作られています。単に一番新しい 1 件を抜き出すより、1 フォルダの作業文脈を素早く掴み直すことを主目的にしています。

`Active`、`Today`、`This Week`、`History` の時間バケットは、フラットな時刻順一覧より見やすい認知の足場として機能します。

## Non-Goals

`ftime` は `fd`、`find`、`eza`、`git status` の置き換えではありません。

- 再帰検索は主目的ではありません。深い階層を探したい時は `fd` や `find` を使います。
- Git の状態確認は主目的ではありません。tracked / untracked を見たい時は `git status` を使います。
- 破壊的操作は対象外です。`ftime` は読み取り専用のままです。
- 汎用的なファイル探索や一発抽出は主目的ではありません。まず文脈を整え、その後に次に見る対象を見つけやすくします。
...
| `ftime`  | 1フォルダの文脈再構築         | バケット + サイズを一目で見られる |
