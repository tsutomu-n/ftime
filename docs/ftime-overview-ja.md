...
更新日: 2025-12-10 / 対象バージョン: v0.1 系（FS Edition）
...
- 拡張子: `--ext` でホワイトリスト。ファイルのみ判定、拡張子なしは除外。大小無視。
...
- 相対時間の文言は英語固定（`just now`, `Yesterday`, `YYYY-MM-DD` など）。
- シンボリックリンクのターゲット表示は TTY のみ。TSV/JSON ではターゲットパスは別フィールド/非表示。
