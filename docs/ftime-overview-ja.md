...
更新日: 2026-03-25 / 対象バージョン: v2.0 系（FS Edition）

この文書は、開発者以外の第三者でも `ftime` の目的・仕様・出力・運用上の注意を短時間で把握できるよう、実装済み機能に基づいて詳細に説明します。設計・試験方針は `ARCHITECTURE.md` / `SPEC-v2.0.md` / `TESTPLAN-v2.0.md` を参照してください。本書はそれらの統合的な読み物です。
...
- v2.0 で `--json` / 拡張子フィルタ / ignore / `--absolute` / `--exclude-dots` / Skew 表示 は実装済み。軽量出自ラベルは `Fresh` のみ実装済み（その他は未着手）。
...
- TTY: 色＋アイコン（オプトイン）＋サイズ列＋時間列 heatmap＋History折り畳み＋各バケット20件上限。future mtime は `Skew` として強調し、末尾に timezone footer を付ける。
- 非TTY: TSV 2カラム（パス, 時間）、全件、ヘッダなし、色なし。`-A` で timezone 付き absolute time を出せる。
- JSON Lines: `--json` で固定フィールドを1行1オブジェクト。TTY判定・色・上限の影響なし。
- dotfiles はデフォルトで表示。不要なら `--exclude-dots` を使う。
- デフォルト除外: `.DS_Store`, `Thumbs.db`（`--no-ignore` で無効化可能）。
...
- `--json`  
  - JSON Lines出力。フィールド凍結:  
    `path`, `bucket`, `mtime`(RFC3339/UTC), `relative_time`, `is_dir`, `is_symlink`, `symlink_target`(symlink解決時のみ), `label`(Fresh時のみ), `size`(regular file のみ)  
  - 色/アイコン/20件上限なし。TTY/非TTYに依存しない。
- `--ext rs,toml`  
  - 拡張子ホワイトリスト（大小無視）。カンマ区切り。ファイルのみ対象。ディレクトリ・拡張子なしは除外。
- `-a, --all` : Historyを展開（TTYのみ、20件上限は維持）。
- `-A, --absolute` : TTY / 非TTY の時間列を `YYYY-MM-DD HH:MM:SS ±HHMM` で表示。
- `--exclude-dots` : dotfiles を除外する。デフォルトでは含める。
- `-I, --icons` : Nerd Fontアイコン（`--features icons` ビルド時、未導入でもフォールバック）。
- `-h, --help` / `-V, --version`

### 環境変数
- `NO_COLOR` : 最優先で色無効（空文字でも無効扱い）。
- `FTIME_FORCE_TTY` : 非TTYでもTTYレイアウトを強制（色の有無は `NO_COLOR` に従う）。Skew / timezone footer / heatmap の確認にも使う。
...
## 5. フィルタリング仕様
- 隠しファイル: デフォルトで含める。不要なら `--exclude-dots` で除外（`.DS_Store` / `Thumbs.db` は built-in ignore の対象）。
...
- デフォルト動作（バケット定義、20件上限、TSVフォーマット、デフォルト ignore）は v2.0 以降も維持予定。
...
## 12. 参考ドキュメント
- 仕様: `docs/SPEC-v2.0.md` / `docs/SPEC-ja.md`
- 設計: `docs/ARCHITECTURE.md` / `docs/ARCHITECTURE-ja.md`
- CLI契約: `docs/CLI.md` / `docs/CLI-ja.md`
- 試験計画: `docs/TESTPLAN-v2.0.md` / `docs/TESTPLAN-ja.md`
...
- Phase 3: 互換性ポリシー凍結、ignore の高度化（gitignore 互換・複数ルート等）、配布パッケージ、SPEC/TESTPLAN v2.0 作成。
