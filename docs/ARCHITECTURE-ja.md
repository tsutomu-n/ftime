...
このドキュメントは `ARCHITECTURE.md` の日本語拡張版です。モジュール分割の意図、依存関係の選択理由、データフロー、失敗時の扱い、将来拡張の設計余地を具体的に記述します。実装・テスト・仕様との整合を最優先し、読み手がコードを開く前に全体像を掴めることを目的とします。
...
├── engine.rs        # スキャン・ソート・バケット化のドメインロジック（拡張子ホワイトリストもここで適用）
├── model.rs         # ドメインデータ構造 (FileEntry, TimeBucket)
├── util/
│   └── time.rs      # 相対時間計算、バケット分類
└── view/
    ├── mod.rs       # ビューの公開モジュール（json は feature gate）
    ├── icon.rs      # バケット見出しのアイコン
    ├── json.rs      # JSON Lines（feature: json）
...
### テキストビュー
- タブ区切り2カラム。機械可読性を最優先。
- `-A` 指定時は timezone 付き absolute time、未指定時は relative time を出す。
...
## 13. スレッド・並列性について
v1.0では単スレッド。`read_dir` とソートのみでCPU負荷は低いため並列化は不要と判断。再帰対応時にはI/Oバウンドになるため Rayon 等の導入を再検討する。
...
- dotfiles はデフォルトで表示されるため、機密ファイル名の露出に注意が必要。必要なら `--exclude-dots` で抑制できる。
...
## 17. ドキュメント間の関係
- 仕様: `SPEC-v1.0.md` / `SPEC-ja.md`
- 設計: 本書と `ARCHITECTURE.md`
- CLI契約: `CLI.md` / `CLI-ja.md`
- テスト: `TESTPLAN-v1.0.md` / `TESTPLAN-ja.md`
...
- **隠しファイルだけのディレクトリ**  
  1) read_dirは要素を返すが `exclude_dots=true` で全スキップ  
  2) entries.len() == 0 → `No recent files found`  
  3) exit code 0  
- **read_link に失敗するシンボリックリンク**  
...
## 27. セキュリティ・プライバシー詳細
- ファイル内容は読まないためデータ漏洩リスクは低い。ただしメタデータ（パス名）は露出するので、機密環境では dotfiles が既定で表示される点に注意する。
...
**1コマンドでビルド＋グローバル化するなら** `cargo install --path . --force` が最短です。  
